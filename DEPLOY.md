...
| `LOGIN_IP_MAX_FAILED` | | 送信元ごとのログイン失敗の上限（既定30回）。営業所がまとめて同じグローバルIPから接続していて誤って止まる場合に上げます |
| `LOGIN_IP_WINDOW_MINUTES` | | 上の回数を数える期間（既定15分） |
...
2. **アプリのログイン**（常時有効・止められません）
   ...
   - ログインに5回失敗すると15分ロック（解除時刻は日本時間で表示）。
     最後の失敗から15分空くと回数は数え直しになります
     （数え直さないと、ログインIDを知っている人が15分に1回の要求で
     そのアカウントを永久にロックし続けられてしまうためです）
   - 送信元ごとにも失敗回数を数え、15分で30回を超えたその送信元を15分止めます。
     利用者ごとのロックは「1アカウントに何度も試す」形しか止められず、
     「多数のアカウントに1回ずつ試す」形は上限に達しないまま通り続けるためです。
     回数は `LOGIN_IP_MAX_FAILED` などで調整できます（上記「環境変数」参照）
...
これに加えて、ブラウザ側で効く保護としてセキュリティヘッダーを付けています
（CSP・`X-Frame-Options` など）。万一どこかに文字列が差し込まれても、
外部から読み込んだスクリプトやインラインスクリプトが動かないようにするものです。

設定場所が2つに分かれている点に注意してください。

| 対象 | 設定場所 |
|---|---|
| 画面ファイル（HTML/JS/CSS） | `vercel.json` の `headers` |
| API（`/api/*`） | `server/auth.js` の `securityHeaders` |

Vercelでは画面ファイルをCDNが直接返すため、Expressのミドルウェアが通りません。
一方、自社サーバー運用（Docker等）ではExpress側が両方を配信します。
**どちらか一方の内容を変えたら、もう一方も必ず同じ内容に合わせてください。**

画面から外部サイトを読み込むもの（現状はGoogle Fontsのみ）を増やす場合は、
CSPの該当ディレクティブにその配信元を足す必要があります。
足し忘れると、その読み込みだけがブラウザ側で黙って失敗します。
