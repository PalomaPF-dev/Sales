...
const SCHEMA_VERSION = '2026-09-10-gain-src';
...
async function beforeSchema() {
  ...
  for (const sql of [
    ...
    // 当月の金額そのもの（合計を合わせるために持つ）
    'ALTER TABLE deals ADD COLUMN master_amount DOUBLE PRECISION',
    ...
    'ALTER TABLE deals ADD COLUMN product_name TEXT',
    // 値上げ交渉（商談結果・最終確定日）。最終確定単価は使われていなかった
    // 取込時代の final_price の枠をそのまま使う
    'ALTER TABLE deals ADD COLUMN nego_result TEXT',
    'ALTER TABLE deals ADD COLUMN final_date TEXT',
    // マスタ登録の作業表に目標値・商談結果の列を足したので作り直す
    // （取込のたびに空にする表なので、落としても失うデータは無い）
    'DROP TABLE IF EXISTS agg_staging',
    ...
    // 売上改善額（マスタ）。売上高（日次・価格実績）のファイルの値をそのまま持つ
    'ALTER TABLE deals ADD COLUMN gain_amount DOUBLE PRECISION',
    'ALTER TABLE deals ADD COLUMN gain_src INTEGER',
    'ALTER TABLE deal_actuals ADD COLUMN gain_amount DOUBLE PRECISION',
    'ALTER TABLE deal_actuals ADD COLUMN gain_src INTEGER',
    'ALTER TABLE sales_progress ADD COLUMN gain_amount DOUBLE PRECISION',
  ...
  try {
    await db.get('SELECT nego_result, nego_note, final_date, product_name, plan_qty, agg_batch,'
      + ' gain_amount, gain_src FROM deal_calc LIMIT 1');
  } catch {
    try { await db.run('DROP VIEW IF EXISTS deal_calc'); } catch { /* 無ければよい */ }
  }
